...
def Inspiration_Extract(sig,filtered=False,vel_c = 0.5,x_hz = 0.2):
...
def breath_cycles(Breaths2,R):
    Br = Breaths2.copy()
    if len(Breaths2)>1:
        Br['In_C'] = R.loc[Breaths2['In']].values
        Br['Ex_C'] = R.loc[Breaths2['Ex']].values
        Br['IDepth'] = R.loc[Breaths2['Ex']].values-R.loc[Breaths2['In']].values
        Br.loc[:len(Breaths2)-2,'EDepth'] = R.loc[Breaths2.loc[:len(Breaths2)-2,'Ex']].values-R.loc[Breaths2.loc[1:,'In']].values
        Br['Insp_T'] = Breaths2['Ex'] - Breaths2['In']
        a = Breaths2['In'].diff()
        Br.loc[:len(Breaths2)-2,'Period_T'] = a[1:].values
        Br['Exp_T'] = Br['Period_T'] - Br['Insp_T']
        Br['IE_Ratio'] = Br['Insp_T']/Br['Exp_T'] 
        Br.loc[:len(Br)-2,'IE_DR'] = Br.loc[:len(Br)-2,'IDepth']/Br.loc[:len(Br)-2,'EDepth'] 
        Br['Insp_V'] = Br['IDepth']/Br['Insp_T']
        Br['Exp_V'] = Br['EDepth']/Br['Exp_T']
...
#     Breaths2['RelED'] = 2*Breaths2['EDepth']/(Breaths2['EDepth'].shift(1)+Breaths2['EDepth'].shift(-1))
        Br['RelID'] = Br['IDepth']/Br['IDepth'].rolling(5,center=True).median()
        Br['RelED'] = Br['EDepth']/Br['EDepth'].rolling(5,center=True).median()
    
        Br.replace([np.inf, -np.inf], np.nan, inplace=True)
        return Br
...
def EQ_Inspiration_Extract(sig,filtered=False,vel_c = 0.35,x_hz = 0.2):
    ...
    if not filtered:
        ...
        newResp['Filt'] = df_sig['Raw']
    newResp['Filt'] = unclip_minor(newResp['Filt'])
    newResp['Diff1'] = diffed(newResp['Filt'])
    ...
    J.iloc[-1] = True
    K = Breaths2['Exp_T'].shift(1)>minInspT
    K.iloc[0] = True
    B.loc[:,'In']= Breaths2.loc[K,'In'].values
